...
def fastrcnn_loss(class_logits, box_regression, labels, regression_targets):
    ...
    # 标签和回归参数进行拼接，传过来的时候labels和regression都是以list列表的形式传过来的，里面的每个元素是batch 中每个图片的对应信息
...
class RoIHeads(torch.nn.Module):
    ...
    def __init__(self,
        ...
        # 计算iou值的方法直接赋值给self.box_similarity
        self.box_similarity = box_ops.box_iou
        # assign ground-truth boxes for each proposal
        # Matcher这个方法就是将proposals划分到正负样本，之前介绍rpn的时候介绍过
        self.proposal_matcher = det_utils.Matcher(
            fg_iou_thresh,  # default: 0.5
            bg_iou_thresh,  # default: 0.5
            allow_low_quality_matches=False)

        # 这个在介绍rpn的时候也介绍过，就是对划分好的正负样本进行采样，总共采样512张，正样本占0.25
    ...
    def add_gt_proposals(self, proposals, gt_boxes):
        ...
        proposals = [
            ...
            # 遍历每张图片的gt_boxes和proposals，将他们拼接在一起，就把gt_boxes也当做proposals的一部分了
    ...
    def select_training_samples(self,
        ...
        # 检查target数据是否为空，检查是否都有boxes和labels
        ...
        # 将gt_boxes拼接到proposal后面，将gt_boxes添加到proposals中，因为正样本的数量很少，这里为了增加正样本的数量
        ...
        for img_id in range(num_images):
            ...
            # 获取对应正负样本的gt box信息。首先获取matched_idxs中当前的图片所对应的所有proposals对应gt_boxes的索引，然后将对应索引出的信息提取出来，这样就得到了这所采样的proposals岁对应的gt的坐标
    ...
    def postprocess_detections(self,
        ...
        # 获取每张图像的预测bbox数量，也就是proposals的数量，预测时候每张图片生成1000个proposals
    ...
    def forward(self,
        ...
        if self.training:
            ...
            # 预测模式直接对预测结果进行后处理，注意预测阶段是不会进行正负样本的划分的，直接对预测的结果进行后处理即可，即直接解析网络的输出参数即可
